...
import { ISDKConfig, IConnectionMethods, IOptions, BTCSignTxInput, BTCSignTxOutput } from '@portis/web3';
import Penpal, { AsyncMethodReturns } from 'penpal';
import { networkAdapter } from '@portis/web3/es/networks';
import { onWindowLoad } from '@portis/web3/es/utils/onWindowLoad';
import { Query } from '@portis/web3/es/utils/query';
import { styles } from '@portis/web3/es/styles';
import { getTxGas } from '@portis/web3/es/utils/getTxGas';
import { INetwork } from '@portis/web3/es/interfaces';
...
export class Portis {
  ...
  constructor(dappId: string, network: string | INetwork, options: IOptions = {}) {
    ...
    this.config = {
      dappId,
      network: networkAdapter(network as INetwork, options.gasRelay),
      version: VERSION,
      scope: options.scope,
  ...
  private _initProvider(options: IOptions) {
    const engine = new ProviderEngine({pollingInterval: 33333});
